...
static bool ParseRenderStateLine(const std::string& Line, FShaderRenderState& State)
{
	...
	if (StartsWith(L, "Cull "))
	{
		...
		if (V == "Back")        State.CullMode = ERHICullMode::Back;
		else if (V == "Front")  State.CullMode = ERHICullMode::Front;
		else if (V == "Off" || V == "None") State.CullMode = ERHICullMode::None;
		return true;
	}
	...
	if (StartsWith(L, "ZTest "))
	{
		...
		if (V == "Never")            State.DepthCompare = ERHICompareOp::Never;
		else if (V == "Less")        State.DepthCompare = ERHICompareOp::Less;
		else if (V == "LEqual")      State.DepthCompare = ERHICompareOp::LessOrEqual;
		else if (V == "Equal")       State.DepthCompare = ERHICompareOp::Equal;
		else if (V == "Greater")     State.DepthCompare = ERHICompareOp::Greater;
		else if (V == "NotEqual")    State.DepthCompare = ERHICompareOp::NotEqual;
		else if (V == "GEqual")      State.DepthCompare = ERHICompareOp::GreaterOrEqual;
		else if (V == "Always")      State.DepthCompare = ERHICompareOp::Always;
...
static std::string FNV1a64Hex(const std::string& Input)
{
	...
	for (char C : Input) { H ^= static_cast<std::uint64_t>(static_cast<unsigned char>(C)); H *= 1099511628211ULL; }
	char Buf[17];
		...
		if (StartsWith(Line, "#include"))
		{
			...
			if (Start != std::string::npos && End != std::string::npos && End > Start)
			{
				...
				if (!Resolved.empty())
				{
					...
					if (IncFile) { std::ostringstream Ss; Ss << IncFile.rdbuf(); Out << Ss.str() << "\n"; continue; }
				}
			}
		}
		Out << Line << '\n';
                                    ...
                                    std::vector<FShaderProperty>& OutProps)
{
	for (; Pos < Source.size(); ++Pos)
	{
		...
		// Skip whitespace
		if (std::isspace(static_cast<unsigned char>(Source[Pos]))) continue;
		...
		if (Name.empty() || Name[0] != '_') { while (Pos < Source.size() && Source[Pos] != '\n') ++Pos; continue; }
		...
		while (!TypeStr.empty() && std::isspace(static_cast<unsigned char>(TypeStr.back()))) TypeStr.pop_back();
                                         ...
                                         std::uint32_t& OutVaryingCount)
{
	...
	if (A2vPos != std::string::npos)
	{
		...
		if (BraceOpen != std::string::npos && BraceClose != std::string::npos)
		{
			...
			while (std::getline(In, Line))
			{
				...
				while (!MemberType.empty() && std::isspace(static_cast<unsigned char>(MemberType.back()))) MemberType.pop_back();
				...
				for (const auto& SI : SemanticTable)
					if (Semantic == SI.Name) { Loc = SI.Location; break; }
			...
			while (A2vEnd < OutGLSL.size() && std::isspace(static_cast<unsigned char>(OutGLSL[A2vEnd]))) ++A2vEnd;
			if (A2vEnd < OutGLSL.size() && OutGLSL[A2vEnd] == ';') ++A2vEnd;
			std::string Stripped;
			...
			// Strip "in a2v v" from vert_main
			{
				for (const char* T : {"in a2v v,", "in a2v v )", "in a2v v)"})
				{
					...
					if (Hit != std::string::npos)
					{
						...
						if (N >= 2 && T[N-2] == ' ' && T[N-1] == ')') EraseN = N - 2;  // keep " )"
						else if (T[N-1] == ')') EraseN = N - 1;                        // keep ")"
	...
	if (V2fPos != std::string::npos)
	{
		...
		if (BraceOpen != std::string::npos && BraceClose != std::string::npos)
		{
			...
			while (std::getline(In, Line))
			{
				...
				if (Semantic != "SV_POSITION") ++OutVaryingCount;
			}

			// Strip struct v2f { ... }; from source
			std::size_t V2fEnd = BraceClose + 1;
			while (V2fEnd < OutGLSL.size() && std::isspace(static_cast<unsigned char>(OutGLSL[V2fEnd]))) ++V2fEnd;
			if (V2fEnd < OutGLSL.size() && OutGLSL[V2fEnd] == ';') ++V2fEnd;
			std::string Stripped;
			Stripped.reserve(OutGLSL.size());
			...
			for (const char* T : {"out v2f o,", "out v2f o )", "out v2f o)"})
			{
				...
				if (Hit != std::string::npos)
				{
					int N = (int)std::char_traits<char>::length(T);
					int EraseN = N;
					if (N >= 2 && T[N-2] == ' ' && T[N-1] == ')') EraseN = N - 2;
					else if (T[N-1] == ')') EraseN = N - 1;
					OutGLSL.erase(Hit, EraseN);
					break;
				}
			}
	...
	// GLSL frag main() must be void; convert to global layout(location=N) out.
	{
		...
		if (FragPos == std::string::npos) FragPos = OutGLSL.find("frag_main(");
		if (FragPos != std::string::npos)
		{
			...
			if (ParenOpen != std::string::npos && ParenClose != std::string::npos)
			{
				...
				for (std::size_t I = 0; I < Tokens.size(); ++I)
				{
					...
					{
						if (!CleanParams.empty()) CleanParams += " ";
	...
	auto FindFuncBody = [&](const std::string& FuncName) -> std::pair<std::size_t, std::size_t>
	{
		...
		if (FuncPos == std::string::npos) FuncPos = OutGLSL.find(FuncName + " (");
		if (FuncPos == std::string::npos) return {std::string::npos, std::string::npos};

		auto BraceOpen = OutGLSL.find('{', FuncPos);
		if (BraceOpen == std::string::npos) return {std::string::npos, std::string::npos};
	...
	for (const auto* Entry : {VertEntry.c_str(), FragEntry.c_str()})
	{
		...
		if (FuncPos == std::string::npos) { Search = std::string(Entry) + " ("; FuncPos = OutGLSL.find(Search); }
		...
		{
			if (OutGLSL[Close] == '{') ++Depth;
			else if (OutGLSL[Close] == '}') --Depth;
		}
		if (Depth != 0) continue; // unmatched braces
                                    ...
                                    std::string& OutInjectedGLSL)
{
	...
	if (VertMainPos == std::string::npos) VertMainPos = OutInjectedGLSL.find("vert_main (");
                                   ...
                                   std::uint32_t& OutVaryingCount)
{
	...
	if (StructPos == std::string::npos) { OutInjectedGLSL = PassSource; return; }

	auto BraceOpen = PassSource.find('{', StructPos);
	auto BraceClose = PassSource.find('}', BraceOpen);
	if (BraceOpen == std::string::npos || BraceClose == std::string::npos) { OutInjectedGLSL = PassSource; return; }
	...
	// Inject inter-stage declarations before vert_main, guarded by stage.
	{
		...
		if (VertPos == std::string::npos) VertPos = OutInjectedGLSL.find("vert_main (");
	...
	auto FragMainPos = OutInjectedGLSL.find("frag_main");
	if (FragMainPos == std::string::npos) FragMainPos = OutInjectedGLSL.find("frag_main ");
	...
	for (const auto& M : Members)
	{
		if (M.bIsSVPosition)
		{
			...
			while (true)
			{
				auto Hit = OutInjectedGLSL.find(Pattern, SearchFrom);
				if (Hit == std::string::npos) break;
				if (FragMainPos != std::string::npos && Hit > FragMainPos) break;
                                      ...
                                      std::string& OutInjectedGLSL)
{
	...
	while (Cur < Params.size())
	{
		...
		while (SemStart < ParamText.size() && std::isspace(static_cast<unsigned char>(ParamText[SemStart]))) ++SemStart;
                                 ...
                                 const std::vector<std::string>& IncludePaths)
{
	...
	if (ShaderStart == std::string::npos) return Result;
                                 ...
                                 const std::string& CacheDir)
{
	...
	for (const auto& Dir : ShaderSearchPaths)
	{
		...
		std::ifstream Test(Candidate);
		if (Test.good()) { FullPath = Candidate; break; }
	}
	if (FullPath.empty()) { MAHO_CORE_ERROR("FShaderDatabase: shader not found: {}", ShaderPath); return false; }

	FShaderFile File = FShaderParser::Parse(FullPath, IncludePaths);
	if (File.Passes.empty()) { MAHO_CORE_ERROR("FShaderDatabase: no passes in {}", ShaderPath); return false; }

	std::vector<std::size_t> ThisPassIndices;
	for (auto& PS : File.Passes)
	{
		if (!FShaderCompiler::Initialize()) return false;
...
void FShaderLoader::ParseShaderSource(const std::string& RawSource, FShaderCompileDesc& OutDesc)
{
	...
	while (std::getline(In, Line))
	{
		...
		if (StartsWith(Line, "#pragma vertex"))
		{
			...
			if (Pos != std::string::npos) OutDesc.VertexEntry = Line.substr(Pos);
		}
		else if (StartsWith(Line, "#pragma fragment"))
		{
			auto Pos = Line.find_first_not_of(" \t", 16);
			if (Pos != std::string::npos) OutDesc.FragmentEntry = Line.substr(Pos);
		}
	}
}
...
FShaderPackage FShaderLoader::LoadShader(const std::string& ShaderPath)
{
	...
	if (FullPath.empty()) { MAHO_CORE_ERROR("FShaderLoader: not found: {}", ShaderPath); return Package; }
	std::string RawSource = ReadFile(FullPath);
	if (RawSource.empty()) { MAHO_CORE_ERROR("FShaderLoader: empty: {}", FullPath); return Package; }
	...
	if (!CachePtr->TryLoad(VtxKey, Package.Vertex.Bytecode))
	{
		...
		else { MAHO_CORE_ERROR("FShaderLoader: VS fail {}", ShaderPath); return Package; }
	}
	else { Package.Vertex.bSuccess = true; CachePtr->TryLoadReflection(VtxKey, Package.Vertex.Reflection); }
	...
	if (!CachePtr->TryLoad(FragKey, Package.Fragment.Bytecode))
	{
		...
		else { MAHO_CORE_ERROR("FShaderLoader: FS fail {}", ShaderPath); return Package; }
	}
	else { Package.Fragment.bSuccess = true; CachePtr->TryLoadReflection(FragKey, Package.Fragment.Reflection); }
