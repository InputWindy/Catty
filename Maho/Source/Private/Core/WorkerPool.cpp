...
struct FWorkerPool::FImpl
{
	...
	{
		MAHO_CORE_INFO("WorkerPool thread {} online", WorkerIndex);
		...
		MAHO_CORE_INFO("WorkerPool thread {} exiting", WorkerIndex);
	}
};
...
bool FWorkerPool::Initialize(std::size_t NumWorkers)
{
	...
	bInitialized = true;
	MAHO_CORE_INFO("WorkerPool initialized ({} threads)", Count);
	return true;
}
...
void FWorkerPool::Shutdown()
{
	...
	if (bInitialized)
	{
		MAHO_CORE_INFO("WorkerPool stopped");
	}
	bInitialized = false;
}
