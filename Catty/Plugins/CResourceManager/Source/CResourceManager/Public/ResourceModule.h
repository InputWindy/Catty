#pragma once

#include "Core/Delegate.h"
#include "Core/Module.h"
#include "CResourceManagerApi.h"
#include "Resource/ResourceManager.h"

#include <vector>

namespace Catty
{

class FScriptSystem;

/** Resource manager module (owns FResourceManager + private FResourceServer). Plugin id: CResourceManager. */
class CATTY_CRESOURCEMANAGER_MODULE_API FResourceModule final : public IModule
{
public:
	const char* GetName() const override { return "CResourceManager"; }

	void GetDependencies(std::vector<std::string>& OutNames) const override
	{
		OutNames.push_back("CGCManager");
	}

	bool OnStage(EModuleStage Stage, FApp& App, FStageContext& Ctx) override;

	[[nodiscard]] FResourceManager& GetResourceManager() { return ResourceManager; }
	[[nodiscard]] const FResourceManager& GetResourceManager() const { return ResourceManager; }

private:
	void OnLuaReady(FScriptSystem& Script);

	FResourceManager ResourceManager;
	FDelegateHandle LuaReadyHandle;
};

} // namespace Catty
