...
struct FThreadedServer::FImpl
{
	...
	const char* ThreadName = "CattyServerThread";

	void ThreadMain(FThreadedServer& Owner)
	{
		CATTY_CORE_INFO("{} thread online", ThreadName);
		...
		CATTY_CORE_INFO("{} thread exiting", ThreadName);
	}
...
bool FThreadedServer::Initialize()
{
	...
	if (!CachedThreadName)
	{
		CachedThreadName = "CattyServerThread";
	}
	if (!CachedLogName)
	{
	...
	CATTY_CORE_INFO("{} started ({})", CachedLogName, CachedThreadName);

	if (!OnInitialize())
	{
		CATTY_CORE_ERROR("{} OnInitialize failed", CachedLogName);
		Shutdown();
		return false;
	}

	bInitialized = true;
	CATTY_CORE_INFO("{} initialized", CachedLogName);
	return true;
}
...
void FThreadedServer::Shutdown()
{
	...
	if (Impl->bRunning)
	{
		...
		Impl->bRunning = false;
		CATTY_CORE_INFO("{} stopped", LogName);
	}
...
void FThreadedServer::Enqueue(std::unique_ptr<FServerTask> Task)
{
...

	{
		...
		{
			CATTY_CORE_WARN("{}::Enqueue ignored (server not running)", LogName);
			return;
		}
