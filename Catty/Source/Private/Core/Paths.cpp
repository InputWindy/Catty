...
#if !defined(CATTY_ENGINE_ROOT)
#	define CATTY_ENGINE_ROOT ""
#endif
...
{
	std::error_code ErrorCode;
	return fs::is_regular_file(Candidate / "Catty" / "CMakeLists.txt", ErrorCode) && !ErrorCode;
}
...
[[nodiscard]] fs::path DetectEngineDir(const fs::path& ProjectRoot)
{
	...
	const fs::path CompiledRoot(CATTY_ENGINE_ROOT);
...
#if defined(_WIN32)
	{
		...
		const DWORD EnvLen = GetEnvironmentVariableA("CATTY_ENGINE_ROOT", EnvBuf, MAX_PATH);
...
#else
	if (const char* EnvRoot = std::getenv("CATTY_ENGINE_ROOT"))
	{
	...
	const fs::path Sibling = ProjectRoot.parent_path() / "Catty";
...
{
	std::error_code ErrorCode;
	const fs::path EngineCattyContent = EngineRoot / "Catty" / "Content";
	if (fs::is_directory(EngineCattyContent, ErrorCode) && !ErrorCode)
	{
		return fs::weakly_canonical(EngineCattyContent, ErrorCode);
	}
	...
	if (!EngineRoot.empty())
	{
		return (EngineRoot / "Catty" / "Content").lexically_normal();
	}
...
void FPaths::Initialize(FEngineConfig& InOutConfig)
{
	...
	const fs::path EngineCattyShaders = EngineRoot / "Catty" / "Shaders";
	std::error_code ErrorCode;
	if (InOutConfig.EngineShadersDir.empty()
		|| !fs::path(InOutConfig.EngineShadersDir).is_absolute())
	{
		if (fs::is_directory(EngineCattyShaders, ErrorCode) && !ErrorCode)
		{
			InOutConfig.EngineShadersDir = fs::weakly_canonical(EngineCattyShaders, ErrorCode).string();
		}
		else
		{
	...
	{
		const fs::path EnginePlugins = EngineRoot / "Catty" / "Plugins";
