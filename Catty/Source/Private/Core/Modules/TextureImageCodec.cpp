	...
	FDecodedImage& Out)
{
	...
	{
		CATTY_CORE_ERROR("TextureImageCodec: CoInitializeEx failed ({})", static_cast<long>(Hr));
		return false;
	}
	...
	if (FAILED(Hr))
	{
		CATTY_CORE_ERROR("TextureImageCodec: WIC factory create failed");
		if (bNeedUninit)
		{
			CoUninitialize();
		}
	...
	if (FAILED(Hr))
	{
		CATTY_CORE_ERROR("TextureImageCodec: WIC CreateDecoderFromStream failed ({})", static_cast<long>(Hr));
		if (bNeedUninit)
		{
			CoUninitialize();
		}
	...
	if (FAILED(Hr))
	{
		CATTY_CORE_ERROR("TextureImageCodec: WIC FormatConverter Initialize failed");
		if (bNeedUninit)
		{
			CoUninitialize();
		}
...
[[nodiscard]] bool EncodeRasterWic(const UTexture& Texture, const std::string& DestinationPath)
{
	if (Texture.GetPixelFormat() != ETexturePixelFormat::RGBA8
		...
		|| Texture.GetHeight() == 0)
	{
		CATTY_CORE_ERROR("TextureImageCodec: WIC encode requires RGBA8 CPU pixels");
		return false;
	}
	...
	FDecodedImage& Out)
{
	...
	{
		CATTY_CORE_ERROR("TextureImageCodec: ktxTexture_CreateFromMemory failed ({})", static_cast<int>(Result));
		return false;
	}

	if (Texture->classId != ktxTexture2_c)
	{
		CATTY_CORE_WARN("TextureImageCodec: non-KTX2 texture; attempting generic load");
	}
	...
	if (!Data || DataSize == 0)
	{
		...
		CATTY_CORE_ERROR("TextureImageCodec: KTX has no image data");
		return false;
	}
...
[[nodiscard]] bool EncodeKtx2(const UTexture& Texture, const std::string& DestinationPath)
{
	...
	{
		CATTY_CORE_ERROR("TextureImageCodec: EncodeKtx2 needs CPU pixels");
		return false;
	}
	...
	{
		CATTY_CORE_ERROR("TextureImageCodec: ktxTexture2_Create failed ({})", static_cast<int>(Result));
		return false;
	}
	...
	if (!bMultiSubImage)
	{
		...
		{
			CATTY_CORE_ERROR("TextureImageCodec: ktxTexture_SetImageFromMemory failed ({})", static_cast<int>(Result));
	...
	if (Result != KTX_SUCCESS)
	{
		CATTY_CORE_ERROR("TextureImageCodec: ktxTexture_WriteToNamedFile failed ({})", static_cast<int>(Result));
		return false;
	}
	return true;
}
#endif // CATTY_WITH_LIBKTX
...
#else
		CATTY_CORE_ERROR("TextureImageCodec: KTX2 requested but CATTY_WITH_LIBKTX is off");
		return false;
#endif
	}
...
#else
	CATTY_CORE_ERROR("TextureImageCodec: raster decode requires Win32 WIC or OpenImageIO");
	return false;
#endif
...
bool EncodeToFile(const UTexture& Texture, const std::string& DestinationPath, bool bOverwrite)
{
	...
	{
		CATTY_CORE_ERROR("TextureImageCodec: destination exists '{}'", DestinationPath);
		return false;
	}
...
#else
		CATTY_CORE_ERROR("TextureImageCodec: KTX2 export requires CATTY_WITH_LIBKTX");
		return false;
#endif
	}
...
#else
	(void)Texture;
	CATTY_CORE_ERROR("TextureImageCodec: raster encode requires Win32 WIC or OpenImageIO");
	return false;
#endif
}
