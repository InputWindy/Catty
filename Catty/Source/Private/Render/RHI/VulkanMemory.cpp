...
bool FVulkanMemoryAllocator::Initialize(VkInstance Instance, VkPhysicalDevice PhysicalDevice, VkDevice Device)
{
	...
	{
		CATTY_CORE_ERROR("FVulkanMemoryAllocator::Initialize: vmaCreateAllocator failed ({})", static_cast<int>(Result));
		Allocator = nullptr;
		return false;
	}

	CATTY_CORE_INFO("VMA allocator created");
	return true;
}
	...
	FRHIMemoryAllocation* OutOpaque)
{
	...
	if (Result != VK_SUCCESS)
	{
		CATTY_CORE_ERROR("FVulkanMemoryAllocator::CreateBuffer failed ({})", static_cast<int>(Result));
		return false;
	}
	...
	FRHIMemoryAllocation* OutOpaque)
{
	...
	if (Result != VK_SUCCESS)
	{
		CATTY_CORE_ERROR("FVulkanMemoryAllocator::CreateImage failed ({})", static_cast<int>(Result));
		return false;
	}
