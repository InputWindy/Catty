...
void FRHIServer::OnShutdown()
{
	ShutdownRHI();
	ResetFrameFence();
	CATTY_CORE_INFO("RHIServer: RHI worker shut down");
}
...
bool FRHIServer::InitializeRHI(FPlatformWindow& InWindow, ERHIBackend Backend)
{
	if (!IsInitialized())
	{
		CATTY_CORE_ERROR("RHIServer::InitializeRHI: server not initialized");
		return false;
	}

	if (!InWindow.HasOsWindow())
	{
		CATTY_CORE_INFO("RHIServer::InitializeRHI: headless; skipping RHI");
		return true;
	}
	...
	if (!NativeHandle)
	{
		CATTY_CORE_ERROR("RHIServer::InitializeRHI: native window handle is null");
		return false;
	}
	...
	if (Width <= 0 || Height <= 0)
	{
		CATTY_CORE_ERROR("RHIServer::InitializeRHI: invalid framebuffer {}x{}", Width, Height);
		return false;
	}
	...
	if (!bOk.load())
	{
		CATTY_CORE_ERROR("RHIServer::InitializeRHI failed");
		return false;
	}

	CATTY_CORE_INFO("RHIServer RHI ready ({}x{})", Width, Height);
	return true;
}
