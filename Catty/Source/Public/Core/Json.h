 ...
 * Example:
 * ```
 *   Catty::FJsonValue Root = Catty::FJsonValue::Object();
 *   Root.SetField("name", Catty::FJsonValue::String("Hero"));
 *   Root.SetField("hp", Catty::FJsonValue::Number(100));
 *
 *   Catty::FJsonValue Skills = Catty::FJsonValue::Array();
 *   Skills.AddElement(Catty::FJsonValue::String("Slash"));
 ...
 *
 *   Catty::FJsonValue Root = Catty::FJsonValue::Object();
 *   Root.SetField("level", Catty::FJsonValue::Number(3));
 *   Doc.SetRoot(Root);
 *   Doc.SaveToFile("Cached/Save.json", true);
 * ```
 */
class CATTY_API FJsonDocument
{
public:
	FJsonDocument();
	~FJsonDocument();
