...
/**
 * RHI worker thread (CattyRHI). Owns FRHI and executes Submit* on the server queue.
 * Short fixed pipeline — not an extensible Extension/DependsPack system.
 * Owned by FRenderServer.
 */
class CATTY_API FRHIServer final : public FThreadedServer
{
public:
	FRHIServer();
	~FRHIServer() override;
...
protected:
	[[nodiscard]] const char* GetServerThreadName() const override { return "CattyRHI"; }
