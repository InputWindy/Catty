#pragma once

#include <Core/Export.h>
#include <Render/RHI/RHIEnums.h>

#include <cstddef>
#include <cstdint>
#include <string>
#include <vector>

namespace Catty
{

class FRHIResourceManager;

struct FRHIMemoryAllocation
{
	void* Native = nullptr;
	void* Mapped = nullptr;
};

class CATTY_API IRHIMemoryAllocator
{
public:
	virtual ~IRHIMemoryAllocator() = default;

	virtual void Free(FRHIMemoryAllocation& Alloc) = 0;
	virtual void* Map(FRHIMemoryAllocation& Alloc) = 0;
	virtual void Unmap(FRHIMemoryAllocation& Alloc) = 0;
};

class CATTY_API FRHIResource
{
public:
	virtual ~FRHIResource() = default;

	FRHIResource(const FRHIResource&) = delete;
	FRHIResource& operator=(const FRHIResource&) = delete;

	[[nodiscard]] virtual const char* GetTypeName() const = 0;
	[[nodiscard]] virtual ERHIResourceKind GetKind() const = 0;
	[[nodiscard]] const std::string& GetDebugName() const { return DebugName; }

protected:
	friend class FRHIResourceManager;

	FRHIResource() = default;

	std::string DebugName;
	std::uint32_t RefCount = 1;
};

struct FRHIBufferDesc
{
	std::uint64_t Size = 0;
	ERHIBufferUsage Usage = ERHIBufferUsage::None;
	ERHIMemoryUsage MemoryUsage = ERHIMemoryUsage::GPUOnly;
};

class CATTY_API FRHIBuffer : public FRHIResource
{
public:
	[[nodiscard]] const char* GetTypeName() const override { return "FRHIBuffer"; }
	[[nodiscard]] ERHIResourceKind GetKind() const override { return ERHIResourceKind::Buffer; }
	[[nodiscard]] virtual const FRHIBufferDesc& GetDesc() const = 0;
};

struct FRHIExtent3D
{
	std::uint32_t Width = 1;
	std::uint32_t Height = 1;
	std::uint32_t Depth = 1;
};

struct FRHITextureDesc
{
	ERHIFormat Format = ERHIFormat::Unknown;
	ERHITextureDimension Dimension = ERHITextureDimension::Tex2D;
	FRHIExtent3D Extent{};
	std::uint32_t MipLevels = 1;
	std::uint32_t ArrayLayers = 1;
	ERHITextureUsage Usage = ERHITextureUsage::None;
	ERHIMemoryUsage MemoryUsage = ERHIMemoryUsage::GPUOnly;
};

class CATTY_API FRHITexture : public FRHIResource
{
public:
	[[nodiscard]] const char* GetTypeName() const override { return "FRHITexture"; }
	[[nodiscard]] ERHIResourceKind GetKind() const override { return ERHIResourceKind::Texture; }
	[[nodiscard]] virtual const FRHITextureDesc& GetDesc() const = 0;
};

struct FRHITextureViewDesc
{
	FRHITexture* Texture = nullptr;
	ERHIFormat Format = ERHIFormat::Unknown;
	std::uint32_t BaseMip = 0;
	std::uint32_t MipCount = 1;
	std::uint32_t BaseArrayLayer = 0;
	std::uint32_t ArrayLayerCount = 1;
};

class CATTY_API FRHITextureView : public FRHIResource
{
public:
	[[nodiscard]] const char* GetTypeName() const override { return "FRHITextureView"; }
	[[nodiscard]] ERHIResourceKind GetKind() const override { return ERHIResourceKind::TextureView; }
};

struct FRHISamplerDesc
{
	ERHIFilter MinFilter = ERHIFilter::Linear;
	ERHIFilter MagFilter = ERHIFilter::Linear;
	ERHIAddressMode AddressU = ERHIAddressMode::Repeat;
	ERHIAddressMode AddressV = ERHIAddressMode::Repeat;
	ERHIAddressMode AddressW = ERHIAddressMode::Repeat;
	float LodBias = 0.0f;
	float MinLod = 0.0f;
	float MaxLod = 1000.0f;
};

class CATTY_API FRHISampler : public FRHIResource
{
public:
	[[nodiscard]] const char* GetTypeName() const override { return "FRHISampler"; }
	[[nodiscard]] ERHIResourceKind GetKind() const override { return ERHIResourceKind::Sampler; }
	[[nodiscard]] virtual const FRHISamplerDesc& GetDesc() const = 0;
};

struct FRHIShaderModuleDesc
{
	ERHIShaderStage Stage = ERHIShaderStage::None;
	const void* Bytecode = nullptr;
	std::size_t BytecodeSize = 0;
};

class CATTY_API FRHIShaderModule : public FRHIResource
{
public:
	[[nodiscard]] const char* GetTypeName() const override { return "FRHIShaderModule"; }
	[[nodiscard]] ERHIResourceKind GetKind() const override { return ERHIResourceKind::ShaderModule; }
};

struct FRHIDescriptorBinding
{
	std::uint32_t Binding = 0;
	ERHIDescriptorType Type = ERHIDescriptorType::UniformBuffer;
	std::uint32_t Count = 1;
	ERHIShaderStage Stages = ERHIShaderStage::None;
};

struct FRHIDescriptorSetLayoutDesc
{
	std::vector<FRHIDescriptorBinding> Bindings;
};

class CATTY_API FRHIDescriptorSetLayout : public FRHIResource
{
public:
	[[nodiscard]] const char* GetTypeName() const override { return "FRHIDescriptorSetLayout"; }
	[[nodiscard]] ERHIResourceKind GetKind() const override { return ERHIResourceKind::DescriptorSetLayout; }
};

struct FRHIPushConstantRange
{
	ERHIShaderStage Stages = ERHIShaderStage::None;
	std::uint32_t Offset = 0;
	std::uint32_t Size = 0;
};

struct FRHIPipelineLayoutDesc
{
	std::vector<FRHIDescriptorSetLayout*> SetLayouts;
	std::vector<FRHIPushConstantRange> PushConstants;
};

class CATTY_API FRHIPipelineLayout : public FRHIResource
{
public:
	[[nodiscard]] const char* GetTypeName() const override { return "FRHIPipelineLayout"; }
	[[nodiscard]] ERHIResourceKind GetKind() const override { return ERHIResourceKind::PipelineLayout; }
};

struct FRHIVertexAttribute
{
	std::uint32_t Location = 0;
	ERHIFormat Format = ERHIFormat::Unknown;
	std::uint32_t Offset = 0;
};

struct FRHIGraphicsPipelineDesc
{
	FRHIShaderModule* VertexShader = nullptr;
	FRHIShaderModule* FragmentShader = nullptr;
	FRHIPipelineLayout* Layout = nullptr;
	ERHIPrimitiveTopology Topology = ERHIPrimitiveTopology::TriangleList;
	std::uint32_t VertexStride = 0;
	std::vector<FRHIVertexAttribute> Attributes;
	ERHICullMode CullMode = ERHICullMode::Back;
	ERHIFillMode FillMode = ERHIFillMode::Solid;
	ERHIFormat ColorFormat = ERHIFormat::B8G8R8A8_UNORM;
	ERHIFormat DepthFormat = ERHIFormat::Unknown;
	bool bDepthTest = false;
	bool bDepthWrite = false;
	ERHICompareOp DepthCompare = ERHICompareOp::Less;
};

class CATTY_API FRHIGraphicsPipeline : public FRHIResource
{
public:
	[[nodiscard]] const char* GetTypeName() const override { return "FRHIGraphicsPipeline"; }
	[[nodiscard]] ERHIResourceKind GetKind() const override { return ERHIResourceKind::GraphicsPipeline; }
};

struct FRHIComputePipelineDesc
{
	FRHIShaderModule* ComputeShader = nullptr;
	FRHIPipelineLayout* Layout = nullptr;
};

class CATTY_API FRHIComputePipeline : public FRHIResource
{
public:
	[[nodiscard]] const char* GetTypeName() const override { return "FRHIComputePipeline"; }
	[[nodiscard]] ERHIResourceKind GetKind() const override { return ERHIResourceKind::ComputePipeline; }
};

struct FRHIDescriptorPoolSize
{
	ERHIDescriptorType Type = ERHIDescriptorType::UniformBuffer;
	std::uint32_t Count = 0;
};

struct FRHIDescriptorPoolDesc
{
	std::uint32_t MaxSets = 0;
	std::vector<FRHIDescriptorPoolSize> PoolSizes;
};

class CATTY_API FRHIDescriptorPool : public FRHIResource
{
public:
	[[nodiscard]] const char* GetTypeName() const override { return "FRHIDescriptorPool"; }
	[[nodiscard]] ERHIResourceKind GetKind() const override { return ERHIResourceKind::DescriptorPool; }
};

class CATTY_API FRHIDescriptorSet : public FRHIResource
{
public:
	[[nodiscard]] const char* GetTypeName() const override { return "FRHIDescriptorSet"; }
	[[nodiscard]] ERHIResourceKind GetKind() const override { return ERHIResourceKind::DescriptorSet; }
};

class CATTY_API FRHIFramebuffer : public FRHIResource
{
public:
	[[nodiscard]] const char* GetTypeName() const override { return "FRHIFramebuffer"; }
	[[nodiscard]] ERHIResourceKind GetKind() const override { return ERHIResourceKind::Framebuffer; }
};

class CATTY_API FRHIRenderPass : public FRHIResource
{
public:
	[[nodiscard]] const char* GetTypeName() const override { return "FRHIRenderPass"; }
	[[nodiscard]] ERHIResourceKind GetKind() const override { return ERHIResourceKind::RenderPass; }
};

class CATTY_API FRHICommandPool : public FRHIResource
{
public:
	[[nodiscard]] const char* GetTypeName() const override { return "FRHICommandPool"; }
	[[nodiscard]] ERHIResourceKind GetKind() const override { return ERHIResourceKind::CommandPool; }
};

class CATTY_API FRHIFence : public FRHIResource
{
public:
	[[nodiscard]] const char* GetTypeName() const override { return "FRHIFence"; }
	[[nodiscard]] ERHIResourceKind GetKind() const override { return ERHIResourceKind::Fence; }
};

class CATTY_API FRHISemaphore : public FRHIResource
{
public:
	[[nodiscard]] const char* GetTypeName() const override { return "FRHISemaphore"; }
	[[nodiscard]] ERHIResourceKind GetKind() const override { return ERHIResourceKind::Semaphore; }
};

class CATTY_API FRHIQueryPool : public FRHIResource
{
public:
	[[nodiscard]] const char* GetTypeName() const override { return "FRHIQueryPool"; }
	[[nodiscard]] ERHIResourceKind GetKind() const override { return ERHIResourceKind::QueryPool; }
};

struct FRHIDescriptorWrite
{
	FRHIDescriptorSet* Set = nullptr;
	std::uint32_t Binding = 0;
	std::uint32_t ArrayIndex = 0;
	ERHIDescriptorType Type = ERHIDescriptorType::UniformBuffer;
	FRHIBuffer* Buffer = nullptr;
	std::uint64_t Offset = 0;
	std::uint64_t Range = 0;
	FRHITextureView* TextureView = nullptr;
	FRHISampler* Sampler = nullptr;
};

} // namespace Catty
