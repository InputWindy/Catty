...
def generate_game_app_cpp(cproject_path: Path, *, log: Any = print) -> Path:
	...
	if plugin_exts:
		...
		for ext in plugin_exts:
			...
			cls = ext["Class"]
			lines.append(
				f"\t\tRegisterExtension<{cls}>(EExtensionPriority::{pri});"
			)
		lines.append("")
	...
	lines.extend(
		[
			...
			"\t\tRegisterExtension<Catty::FEditorLayer>(EExtensionPriority::Overlay);",
			"#endif",
			"\t\tRegisterExtension<Catty::FScriptLayer>(EExtensionPriority::Overlay);",
			"\t\treturn true;",
			"\t}",
			"};",
			"",
	...
	out_dir = project_dir / "Source" / "Generated"
	out_dir.mkdir(parents=True, exist_ok=True)
	out_path = out_dir / f"{project_name}App.cpp"
	out_path.write_text("\n".join(lines), encoding="utf-8", newline="\n")
	log(f"[Catty] Generated App: {out_path}")
	return out_path
